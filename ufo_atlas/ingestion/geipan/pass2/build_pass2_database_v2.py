...
import argparse, csv, hashlib, json, shutil, sqlite3, zipfile
...
def main():
    ...
    parts=sorted(Path(args.jsonl_dir).rglob('*.jsonl'))
    ...
    ids=[r['source_case_id'] for r in rows]; urls=[r['case_url'] for r in rows]
    ...
    for r in rows:
        desc_en=(r.get('description_en') or '').strip(); summary_en=(r.get('summary_en') or '').strip(); bilingual=(r.get('bilingual_description') or '').strip()
    ...
    info={
      'pass2_version':PASS2_VERSION,'case_count':'3381','public_encounter_classes':'CE1|CE2|CE3|CE4|CE5',
    ...
    con.executemany('INSERT INTO pass2_release_info(key,value) VALUES (?,?)',info.items()); con.commit()

    integrity=con.execute('PRAGMA integrity_check').fetchone()[0]; fk=con.execute('PRAGMA foreign_key_check').fetchall()
    ...
    missing_en=con.execute("SELECT count(*) FROM case_pass2 WHERE trim(description_en)='' OR trim(summary_en)='' OR trim(phenomenon_type_en)='' ").fetchone()[0]
    missing_fr=con.execute("SELECT count(*) FROM case_pass2 WHERE trim(description_fr)='' OR instr(bilingual_description,description_fr)=0").fetchone()[0]
    english_not_first=con.execute("SELECT count(*) FROM case_pass2 WHERE substr(bilingual_description,1,length(description_en))<>description_en").fetchone()[0]
    source_count=con.execute('SELECT count(*) FROM case_page').fetchone()[0]; pass2_count=con.execute('SELECT count(*) FROM case_pass2').fetchone()[0]
    con.close()
    checks={
      'status':'PASS' if integrity=='ok' and not fk and missing_en==0 and missing_fr==0 and english_not_first==0 and source_count==pass2_count==3381 and sum(census.values())==3381 else 'FAIL',
      'sqlite_integrity':integrity,'foreign_key_violations':len(fk),'source_case_count':source_count,'pass2_case_count':pass2_count,
      'encounter_class_census':census,'missing_english_fields':missing_en,'missing_french_originals':missing_fr,'english_not_first':english_not_first,
    ...
    print(json.dumps(checks,indent=2)); print(zip_path)
